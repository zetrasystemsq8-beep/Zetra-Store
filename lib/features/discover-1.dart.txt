...
/// Role choice tracking provider
final roleChoiceMadeProvider = FutureProvider<bool>((ref) async {
  final prefs = await SharedPreferences.getInstance();
  return prefs.getBool('role_choice_made') ?? false;
});
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      try {
        ...
        // Handle error silently — user will just see the home screen
        debugPrint('Error checking role choice: $e');
      }
    });
  }
